...
Public Class Service
    ...
    Public Function PublishDMSDoc(ByVal DocId As String, ByVal ContactId As String, ByVal NotifyFlg As String, _
        ...
        SqlS = "INSERT INTO DMS.dbo.Document_Associations(created_by, last_upd_by, association_id, doc_id, fkey, pr_flag, access_flag, reqd_flag, expiration) " & _
        "SELECT TOP 1 1, 1,3," & DocId & ",'" & ContactId & "','Y','Y','" & ReqdFlag & "'," & temp & " " & _
        "FROM DMS.dbo.Document_Associations " & _
        "WHERE NOT EXISTS (SELECT doc_id FROM DMS.dbo.Document_Associations " & _
        "WHERE doc_id=" & DocId & " AND fkey='" & ContactId & "' AND association_id=3)"
        ...
        Select Case Domain
            ...
            Case Else
                loginpath = ""
        End Select
        If Debug = "Y" Then mydebuglog.Debug("  Loginpath: " & loginpath)
        ...
        If NotifyFlg = "Y" And results <> "Failure" Then
            If EMAIL_ADDR <> "" And ReplyTo <> "" Then
                ...
                If UID <> "" Then
                    ...
                    Body = Body & "To access this document, click on the following link:" & _
                ...
                If Debug = "Y" Then mydebuglog.Debug("  > Email XML: " & sXML)
...
CloseOut:
    ...
    Public Function UpdDMSDocCount(ByVal CONTACT_ID As String, ByVal TRAINER_NUM As String, ByVal PART_ID As String, _
        ...
        Try
            ...
            If RO_ConnS = "" Then ConnS = "server=;Min Pool Size=3;Max Pool Size=5;Connect Timeout=10;ApplicationIntent=ReadOnly;"
            temp = System.Configuration.ConfigurationManager.AppSettings.Get("UpdDMSDocCount_debug")
            If temp = "Y" And Debug <> "T" Then Debug = "Y"
        Catch ex As Exception
            errmsg = errmsg & "Unable to get defaults from web.config. " & vbCrLf
        ...
        Try
            d_ConnS = System.Configuration.ConfigurationManager.ConnectionStrings("dms").ConnectionString
            If d_ConnS = "" Then d_ConnS = "server=;Min Pool Size=3;Max Pool Size=5;Connect Timeout=10;database=DMS"
        Catch ex As Exception
            errmsg = errmsg & "Unable to get defaults from web.config. " & vbCrLf
            results = "Failure"
            GoTo CloseOut2
...
CloseOut2:
    ...
    Public Function SaveDMSDocAssoc(ByVal DocId As String, ByVal Association As String, _
        ...
        Try
            d_ConnS = System.Configuration.ConfigurationManager.ConnectionStrings("dms").ConnectionString
            If d_ConnS = "" Then d_ConnS = "server=;database=DMS"
...
CloseOut2:
    ...
    Public Function SaveDMSDoc(ByVal DImage As String, ByVal DocId As String, ByVal ReportId As String, ByVal ItemName As String, ByVal DFileName As String, ByVal DURL As String, _
        ...
        Try
            ...
            If ConnS = "" Then ConnS = "server=;database=siebeldb"
...
CloseOut2:
    ...
    Public Function UserLogin(ByVal ContactId As String, ByVal UserId As String, ByVal SessionId As String, _
        ...
        Try
            ...
            If d_ConnS = "" Then d_ConnS = "server=;database=DMS"
            RO_ConnS = System.Configuration.ConfigurationManager.ConnectionStrings("hcidbro").ConnectionString
            If RO_ConnS = "" Then RO_ConnS = "server=;Min Pool Size=3;Max Pool Size=5;Connect Timeout=10;ApplicationIntent=ReadOnly;"
                ...
                If Not RO_dr Is Nothing Then
                    While RO_dr.Read()
                        Try
                            ...
                            If Debug = "Y" Then mydebuglog.Debug("  > Located Session Id: " & DMS_SESSION_ID)
                        Catch ex As Exception
                            results = "Failure"
        ...
        If DMS_SESSION_ID = "" Then
            ...
            If Debug = "Y" Then mydebuglog.Debug("  Removing old session: " & SqlS)
            Try
                d_cmd.CommandText = SqlS
                returnv = d_cmd.ExecuteNonQuery()
            Catch ex As Exception
            ...
            SqlS = "INSERT INTO  DMS.dbo.User_Sessions(user_id, session_key, machine_id) " & _
            "VALUES('" & UserId & "','" & SessionId & "','CM')"
            If Debug = "Y" Then mydebuglog.Debug("  Create user session record: " & SqlS)
            Try
                d_cmd.CommandText = SqlS
                returnv = d_cmd.ExecuteNonQuery()
            Catch ex As Exception
        ...
        If Debug = "Y" Then mydebuglog.Debug("  Getting users user access id: " & SqlS)
        Try
            RO_cmd.CommandText = SqlS
            RO_dr = RO_cmd.ExecuteReader()
            If Not RO_dr Is Nothing Then
        ...
        If SubId <> "" Then
            ...
            If Debug = "Y" Then mydebuglog.Debug("  Getting subscription user access id: " & SqlS)
            Try
                RO_cmd.CommandText = SqlS
                RO_dr = RO_cmd.ExecuteReader()
                If Not RO_dr Is Nothing Then
        ...
        If Domain <> "" Then
            SqlS = "SELECT UA.row_id " & _
                ...
                "INNER JOIN DMS.dbo.Groups G ON G.row_id=UA.access_id " & _
                "WHERE UA.type_id='G' AND G.name='" & Domain & "'"
            If Debug = "Y" Then mydebuglog.Debug("  Getting domain user access id: " & SqlS)
            Try
                RO_cmd.CommandText = SqlS
                RO_dr = RO_cmd.ExecuteReader()
                If Not RO_dr Is Nothing Then
...
CloseOut2:
        ' ============================================
        ' Close the log file if any
        ltemp = results & " for user " & UserId & ", in " & Domain & " domain"
    ...
    Public Function GetDocument(ByVal RegId As String, ByVal UserId As String, ByVal Type As String, ByVal Debug As String, ByVal Asset As String, ByVal DocId As String) As Byte()
        ...
        Try
            ...
            If ConnS = "" Then ConnS = "server=;database=siebeldb;ApplicationIntent=ReadOnly"
            d_ConnS = System.Configuration.ConfigurationManager.ConnectionStrings("dms").ConnectionString
            If d_ConnS = "" Then d_ConnS = "server=;database=DMS;ApplicationIntent=ReadOnly"
...
CloseOut2:
    ...
    Public Function EmpQuery(ByVal EmpId As String, ByVal ConId As String, ByVal FunctionId As String, ByVal TranId As String, ByVal Query As String, ByVal DMSFlag As String,
        ...
        Try
            ConnS = System.Configuration.ConfigurationManager.ConnectionStrings("hcidb").ConnectionString
            If ConnS = "" Then ConnS = "server=;database=siebeldb;Min Pool Size=3;Max Pool Size=5"
            d_ConnS = System.Configuration.ConfigurationManager.ConnectionStrings("dms").ConnectionString
            If d_ConnS = "" Then d_ConnS = "server=;database=DMS;Min Pool Size=3;Max Pool Size=5"
...
DropTables:
                ...
                If FunctionId = "GetContent" And Tmp_TableName <> "" And TableName <> "" Then
                    SqlS = "SELECT DISTINCT row_id, name, description, type, category=left(cat.list, len(cat.list)-1), " &
...
CloseOut2:
        ...
        myeventlog.Info("CMQuery : Results: " & ltemp)
        If Debug = "Y" Or (Logging = "Y" And Debug <> "T") Then
            Try
                ' Log results
